...
// pitch_code that signifies one of seven pitch types:
...
).map(csvTransform).batch(100)

const trainingValidationData = tf.data.csv(
  TRAIN_DATA_PATH,
  {
    columnConfigs: {
      pitch_code: { isLabel: true }
...
const testValidationData = tf.data.csv(
  TEST_DATA_PATH,
  {
    columnConfigs: {
      pitch_code: { isLabel: true }
...
async function evaluate (userTestData) {
  const results = {}
